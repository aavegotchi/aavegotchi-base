...
describe("SwapAndBuyERC721 Integration Test", function () {
  ...
  before(async function () {
    ...
    if (zRouterCode === "0x") {
      throw new Error(`❌ zRouter not found at ${zRouterAddress} - this should not happen!`);
    }
  ...
  async function fetchActiveListing(): Promise<ERC721Listing> {
    ...
    console.log("🔍 Fetching real ERC721 listings from Aavegotchi Base subgraph...");
  ...
  async function fetchActiveListing_old(): Promise<ERC721Listing> {
    const query = `
  ...
  describe("USDC to GHST Swap and Purchase", function () {
    it("Should swap USDC for GHST and purchase NFT", async function () {
      ...
      console.log(
        `📉 Min GHST out (required): ${ethers.utils.formatEther(totalCost)} GHST`
      );
